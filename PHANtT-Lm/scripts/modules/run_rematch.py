...
def determine_general_statistics(outdir, sample_data, minimum_gene_coverage, minimum_gene_identity):
    ...
    with open(os.path.join(outdir, 'rematchModule_report.txt'), 'wt') as writer:
        ...
        writer.write('\t'.join([sample_data['header'], str(round(sample_data['gene_coverage'], 2)), str(round(sample_data['gene_mean_read_coverage'], 2)), str(round(sample_data['gene_low_coverage'], 2)), str(sample_data['gene_number_positions_multiple_alleles']), str(round(sample_data['gene_identity'], 2))]) + '\n')
