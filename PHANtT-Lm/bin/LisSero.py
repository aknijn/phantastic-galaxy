...
parser.add_argument('--version', action='version', version=
    ...
    'Updated 29-Nov-2018 by Arnold Knijn\n'
    'Dependencies: Python 3.x, EMBOSS PrimerSearch, isPcr\n'
    '========================================')
    ...
    for f in args.fasta:
        ...
        for line in proc.stdout:
            if not line.strip():                        # Ignore blank lines
                continue
            if line.startswith(b"Primer name"):            # Retrieve primer name
                name = int(line.split()[2])
            if line.startswith(b"\tAmplimer length"):    # Check amplicon length against expected size in dictionary
        ...
        b = list(map(int, a))                        # Ensure all values are integers
