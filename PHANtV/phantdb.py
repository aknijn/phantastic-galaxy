...
class IridaDb:
    ...
    def user_in_role(self, username, userrole):
        sql = 'SELECT COUNT(*) from user_group_member INNER JOIN user on(user.id=user_group_member.user_id) INNER JOIN user_group on(user_group.id=user_group_member.group_id) WHERE email="%s" and name="%s"'
