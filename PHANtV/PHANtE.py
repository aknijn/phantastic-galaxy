...
def getMetadata(inputfiles, inuser, insql):
...
def main():
    ...
    if inspecies == 'Coronavirus':
        ...
        sql_species = "select sample.id AS id,sample.sampleName AS NomeCampione,date_format(sample.collectionDate,'%Y-%m-%d') AS DataCampione,date_format(sample.arrivalDate,'%Y-%m-%d') AS DataArrivoCampione,date_format(sample.createdDate,'%Y-%m-%d') AS DataCaricamento,sample.strain AS CodiceInterno,sample.description AS Descrizione,max(case when metadata_entry.field_id = 8 then metadata_entry.value end) AS QC,sample.geographicLocationName AS Regione,sample.geographicLocationName2 AS Provincia,sample.geographicLocationName3 AS Comune,sample.isolationSource AS OrigineIsolato,sample.patientAge AS FasciaEta,sample.patientVaccinationNumber AS NumeroVaccinazioni,sample.patientVaccinationDate AS DataUltimaVaccinazione,sample.collectedBy AS Ospedale,user_group.name AS Laboratorio,sample.isolate AS CondizioneClinica,max(case when metadata_entry.field_id = 3 then metadata_entry.value end) AS Antigen_O,max(case when metadata_entry.field_id = 12 then metadata_entry.value end) AS Antigen_H,max(case when metadata_entry.field_id = 11 then metadata_entry.value end) AS MLST_ST,max(case when metadata_entry.field_id = 14 then metadata_entry.value end) AS stx1,max(case when metadata_entry.field_id = 15 then metadata_entry.value end) AS stx2,max(case when metadata_entry.field_id = 6 then metadata_entry.value end) AS 'stx_subtype',max(case when metadata_entry.field_id = 10 then metadata_entry.value end) AS 'eae',max(case when metadata_entry.field_id = 4 then metadata_entry.value end) AS ehxa,max(case when metadata_entry.field_id = 22 then metadata_entry.value end) AS Virulotipi,max(case when metadata_entry.field_id = 9 then metadata_entry.value end) AS cgMLST_genes_mapped,max(case when metadata_entry.field_id = 7 then metadata_entry.value end) AS Cluster_Id,max(case when qc_entry.total_bases = 0 then 1 else qc_entry.total_bases DIV 29811 end) AS Coverage,lab_users.username AS email,sequence_file_pair_files.files_id AS files_id from metadata_entry join sample on(metadata_entry.sample_id = sample.id) join project_sample on(sample.id = project_sample.sample_id) join user on(user.username = sample.sequencedBy) join user_group_member ugm on(user.id = ugm.user_id) join user_group on(ugm.group_id = user_group.id) join user_group_member lgm on(user_group.id = lgm.group_id) join user lab_users on(lgm.user_id = lab_users.id) join sample_sequencingobject on(sample.id = sample_sequencingobject.sample_id) join sequence_file_pair_files on(sample_sequencingobject.sequencingobject_id = sequence_file_pair_files.pair_id) join qc_entry on(sample_sequencingobject.sequencingobject_id = qc_entry.sequencingObject_id) where project_sample.project_id = 1 and user_group.id > 5 and ugm.role = 'GROUP_MEMBER' and lgm.role = 'GROUP_MEMBER' and "
        ...
        sql_species = "select sample.id AS id,sample.sampleName AS NomeCampione,date_format(sample.collectionDate,'%Y-%m-%d') AS DataCampione,date_format(sample.arrivalDate,'%Y-%m-%d') AS DataArrivoCampione,date_format(sample.createdDate,'%Y-%m-%d') AS DataCaricamento,sample.strain AS CodiceInterno,sample.description AS Descrizione,max(case when metadata_entry.field_id = 8 then metadata_entry.value end) AS QC,sample.geographicLocationName AS Regione,sample.geographicLocationName2 AS Provincia,sample.geographicLocationName3 AS Comune,sample.isolationSource AS OrigineIsolato,sample.patientAge AS FasciaEta,sample.patientVaccinationNumber AS NumeroVaccinazioni,sample.patientVaccinationDate AS DataUltimaVaccinazione,sample.collectedBy AS Ospedale,user_group.name AS Laboratorio,max(case when metadata_entry.field_id = 19 then metadata_entry.value end) AS Serogroup,max(case when metadata_entry.field_id = 21 then metadata_entry.value end) AS Serotype,max(case when metadata_entry.field_id = 20 then metadata_entry.value end) AS Amplicons,max(case when metadata_entry.field_id = 11 then metadata_entry.value end) AS MLST_ST,max(case when metadata_entry.field_id = 17 then metadata_entry.value end) AS MLST_CC,max(case when metadata_entry.field_id = 18 then metadata_entry.value end) AS 'MLST_Lineage',max(case when metadata_entry.field_id = 9 then metadata_entry.value end) AS 'cgMLST_genes_mapped',max(case when metadata_entry.field_id = 7 then metadata_entry.value end) AS Cluster_Id,max(case when qc_entry.total_bases = 0 then 1 else qc_entry.total_bases DIV 29811 end) AS Coverage,lab_users.username AS email,sequence_file_pair_files.files_id AS files_id from metadata_entry join sample on(metadata_entry.sample_id = sample.id) join project_sample on(sample.id = project_sample.sample_id) join user on(user.username = sample.sequencedBy) join user_group_member ugm on(user.id = ugm.user_id) join user_group on(ugm.group_id = user_group.id) join user_group_member lgm on(user_group.id = lgm.group_id) join user lab_users on(lgm.user_id = lab_users.id) join sample_sequencingobject on(sample.id = sample_sequencingobject.sample_id) join sequence_file_pair_files on(sample_sequencingobject.sequencingobject_id = sequence_file_pair_files.pair_id) join qc_entry on(sample_sequencingobject.sequencingobject_id = qc_entry.sequencingObject_id) where project_sample.project_id = 1 and user_group.id > 5 and ugm.role = 'GROUP_MEMBER' and lgm.role = 'GROUP_MEMBER' and "
    else:
        csv_header = []
        sql = "select * from sample LIMIT 0"
    args = parser.parse_args()
    metadata = getMetadata(args.input_files, args.user.replace("__at__", "@"), sql_species)
    if metadata:
        # Create csv
