...
class IridaDb:
    def __init__(self, species):
        ...
        if species == 'Escherichia coli' or species == 'Shiga toxin-producing Escherichia coli':
            ...
            self._species = 'Influenza A/Influenza B'
            self._configFile = TOOL_DIR + '/../recovery.conf'
            self._allele_header_file = ''
        else:
    ...
    def metadata_for_export(self, user, fileIds):
        sql_where = " (lab_users.username = '%s' or '%s'='@iss.it') and files_id in (%s) group by files_id"
        if self._species == 'Coronavirus':
            ...
            sql = sql_species + sql_where
        if self._species == 'Influenza A/Influenza B':
            sql_species = "select sample.id AS id,sample.sampleName AS NomeCampione,date_format(sample.collectionDate,'%%Y-%%m-%%d') AS DataCampione,date_format(sample.arrivalDate,'%%Y-%%m-%%d') AS DataArrivoCampione,date_format(sample.createdDate,'%%Y-%%m-%%d') AS DataCaricamento,sample.strain AS CodiceInterno,sample.externalId AS CodiceGISAID,sample.description AS Descrizione,max(case when metadata_entry.field_id = 8 then metadata_entry.value end) AS QC,sample.geographicLocationName AS Regione,sample.geographicLocationName2 AS Provincia,sample.isolationSource AS OrigineIsolato,sample.patientAge AS FasciaEta,sample.patientVaccinationDate AS DataUltimaVaccinazione,sample.collectedBy AS Ospedale,user_group.name AS Laboratorio,f_GetIsolateValue(1,sample.isolate) AS Sesso,f_GetIsolateValue(2,sample.isolate) AS Provenienza,f_GetIsolateValue(3,sample.isolate) AS TerapiaIntensiva,f_GetIsolateValue(4,sample.isolate) AS CondizioneRischio,f_GetIsolateValue(5,sample.isolate) AS Immunodepresso,f_GetIsolateValue(6,sample.isolate) AS TerapiaAntivirale,max(case when metadata_entry.field_id = 11 then metadata_entry.value end) AS Influenza_tipo,max(case when metadata_entry.field_id = 15 then metadata_entry.value end) AS Clade,max(case when metadata_entry.field_id = 10 then metadata_entry.value end) AS N_sottotipo,max(case when metadata_entry.field_id = 12 then metadata_entry.value end) AS H_sottotipo,max(case when metadata_entry.field_id = 13 then metadata_entry.value end) AS HA_aa_MUT,max(case when metadata_entry.field_id = 14 then metadata_entry.value end) AS HA_Frame_shift,max(case when metadata_entry.field_id = 16 then metadata_entry.value end) AS HA_Reference,lab_users.username AS email,sequence_file_pair_files.files_id AS files_id from metadata_entry join sample on(metadata_entry.sample_id = sample.id) join project_sample on(sample.id = project_sample.sample_id) join user on(user.username = sample.sequencedBy) join user_group_member ugm on(user.id = ugm.user_id) join user_group on(ugm.group_id = user_group.id) join user_group_member lgm on(user_group.id = lgm.group_id) join user lab_users on(lgm.user_id = lab_users.id) join sample_sequencingobject on(sample.id = sample_sequencingobject.sample_id) join sequence_file_pair_files on(sample_sequencingobject.sequencingobject_id = sequence_file_pair_files.pair_id) join qc_entry on(sample_sequencingobject.sequencingobject_id = qc_entry.sequencingObject_id) where project_sample.project_id = 1 and user_group.id > 5 and ugm.role = 'GROUP_MEMBER' and lgm.role = 'GROUP_MEMBER' and "
            sql = sql_species + sql_where
    ...
    def header_for_export(self):
        ...
        if self._species == 'Influenza A/Influenza B':
            csv_header = ['id','NomeCampione','DataCampione','DataArrivoCampione','DataCaricamento','CodiceInterno','CodiceGISAID','Descrizione','QC','Regione','Provincia','OrigineIsolato','FasciaEta','DataUltimaVaccinazione','Ospedale','Laboratorio','Sesso','Provenienza','TerapiaIntensiva','CondizioneRischio','Immunodepresso','TerapiaAntivirale','Influenza_tipo','Clade','N_sottotipo','H_sottotipo','HA_aa_MUT','HA_Frame_shift','HA_Reference','email','files_id']
        elif self._species == 'Escherichia coli':
